...
# Exécuter mysqldump pour exporter la base de données
docker exec -i $container_id mysqldump -u root --password=$MYSQL_PASSWORD $DATABASE_NAME > $CURRENT_DIR/$DUMP_FILE

echo "Base de données exportée dans le fichier $DUMP_FILE."
